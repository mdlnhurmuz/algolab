...
void testcase() {
    ...
    const vertex_desc v_target = N - 1;

    for (int i = 1; i < w; i++) {
        adder.add_edge(i, w - 1 + i, 1);
        //cout << "Pun muchie de la " << i << " la " << w - 1 + i << "\n";
    }
    ...
    for (int i = 0; i < v.size(); i++) {
        ...
        if (s == v_source) {
            if (e == w) {
                adder.add_edge(v_source, v_target, 1);
            }
            else adder.add_edge(v_source, e, 1);
            //cout << "Pun muchie de la " << v_source << " la " << e << "\n";
        } else if (e == w) {
            adder.add_edge(w - 1 + s, v_target, 1);
        } else {
            adder.add_edge(w - 1 + s, e, 1);
        }
    }
