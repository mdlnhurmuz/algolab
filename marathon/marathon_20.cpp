...
void testcase() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        e = boost::add_edge(points.second, points.first, G2).first; weights2[e]=length;
    }

    vector<int> dist_from_source = dijkstra_dist(G1, s);  
    vector<int> dist_from_finish = dijkstra_dist(G2, f);

    /*for (int i = 0; i < dist_from_source.size(); i++) {
        cout << dist_from_source[i] << "\n";
    }  
    cout << "\n";

    for (int i = 0; i < dist_from_finish.size(); i++) {
        cout << dist_from_finish[i] << "\n";
    }  
